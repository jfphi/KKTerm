...
**Screenshots Module**:
A built-in Activity Rail Module that captures screenshots into a user-configurable library folder and presents thumbnail or details views. Capture modes are interactive region, interactive window pick, and full screen (all monitors); they are reachable from the single-row Module header toolbar, the tray icon menu, and configurable global hotkeys, and all three paths share one frontend capture bridge. Module-header captures can use an Instant, 3, 5, 15, 30, or 60 second delay; tray and global-hotkey captures stay immediate. Captures save as PNG by default (JPEG optional) on Windows; on other platforms the Module shows the library read-only. The library supports persisted sort/group controls, multi-selection, non-destructive batch resize/conversion, delete, and a unified Fit/zoom viewer/editor for arrows, rectangles, ellipses, text, and mosaic regions. Single items also support copy to clipboard, open in the default app, reveal in folder, and rename. Folder, format, and hotkeys live in Settings → Screenshots; tray capture commands display their current enabled hotkeys. Distinct from the per-Pane Workspace screenshot menu, which copies captures to the clipboard or the AI Assistant and never touches the library.
